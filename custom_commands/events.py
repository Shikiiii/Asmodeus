                    ...
                    for key, value in starboardMessages.items():
                        if int(key) == reaction.message.channel.id:
                            if int(value) == reaction.message.id:
                                return
                    embed = discord.Embed(description="{}\n\n[JUMP TO MESSAGE](https://discordapp.com/channels/{guild.id}/{chan.id}/{msg.id})".format(reaction.message.content, reaction.message.guild.id, reaction.channel.id, reaction.message.id), color=0xffff00)
